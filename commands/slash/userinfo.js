        ...
        try {
            ...
            require("../../components/database").get(`/${interaction.guild.id}/levels/${member.id}`, client).then(level => {
                ...
                .addFields(
                    ...
                    { name: `<:boost:1200817314740842616> __**Level:**__`, value: `> ${(client.data.level.has(member.id) ? `${client.data.level.get(member.id).level} (${client.data.level.get(member.id).xp}/${client.data.level.get(member.id).maxxp}` : `1 (0/100`)})`},
